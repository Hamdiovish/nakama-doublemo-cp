...
func (r *LocalSessionRegistry) SingleSession(ctx context.Context, tracker Tracker, userID, sessionID uuid.UUID) {
	...
	sessionIDs := tracker.ListByStream(PresenceStream{Mode: StreamModeNotifications, Subject: userID}, true, true)
	for _, foundSessionID := range sessionIDs {
		if foundSessionID.ID.SessionID == sessionID {
			// Allow the current session, only disconnect any older ones.
			continue
		}

		message := &rtapi.Envelope{Message: &rtapi.Envelope_Notifications{
			Notifications: &rtapi.Notifications{
				Notifications: []*api.Notification{
					{
						Id:         uuid.Must(uuid.NewV4()).String(),
		...
		session, ok := r.sessions.Load(foundSessionID.ID.SessionID)
		if ok {
			// No need to remove the session from the map, session.Close() will do that.
			session.Close("server-side session disconnect", runtime.PresenceReasonDisconnect, message)
			return
		}
